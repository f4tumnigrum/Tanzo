...
- **代价的边界（易被误读）**：上一条"可接受"只对**用户手动**的低频操作成立。
  charter 的渲染条件是 `deriveStatus(goal) ∈ {active, budget_limited}`
  （`src/shared/goal.ts:49`），而 `deriveStatus` 是 `outcome/limit/userState`
  字段的纯投影 —— 意味着 charter 的"出现/消失"不止由用户触发，也由**系统自动
  状态翻转**触发（见 §8 的两条 cache 风险）。charter 一旦跨过
  {active,budget_limited}（在）↔ {paused,usage_limited,complete,blocked}（消失）
  这条边界，稳定 system 前缀就改写一次、Anthropic 1h 锚点整段失效。
  这正是"进/出 goal 模式丢缓存"的**根因**：不是缓存实现有 bug，而是把一段会随
  生命周期出现/消失的内容放进了本应恒定的缓存前缀，且它恰好是 order 最大的稳定段
  （40），压在 1h 断点上。
...
- charter 进入 stable system 段 → 被 Anthropic `stableBoundary` 1h 锚点、
  OpenAI key 路由、DeepSeek 磁盘 prefix-unit 匹配、Google 隐式缓存天然覆盖，
  **不新增任何 cache 断点**（Anthropic 4 断点预算不变）。
...
- **自动状态翻转把"低频"放大成"经常"（cache 风险，上一条的前提缺口）**：
  上一条只覆盖用户操作。但 charter 还随 `deriveStatus` 自动增删 ——
  `usage_limited`（`goal.limit === 'usage'`，provider/账户级用量额度）在额度耗尽
  时让 charter 消失、额度恢复后又出现,这类翻转**不是用户操作、不受"低频"约束**,
  每翻一次砸一次前缀缓存。`paused`↔`active`（用户暂停/恢复）同理。相反,
  `active`↔`budget_limited` **不**触发失效 —— 两态 charter 均在、内容相同、前缀
  不变（这点常被误判为抖动源,实非）。缓解方向（不在本设计强制,列为已知权衡）：
  (a) 对 `usage_limited` 做滞回/宽限,避免临界反复翻;(b) 若翻转频繁成为实测痛点,
  再评估把 charter 移出 stable system（改 injection 常驻）—— 代价是丢掉"模型每步
  在 system 看到目标"与"稳态零边际成本",与 I1/I7 初衷冲突,非当前取舍。
- **ledger 不感知 charter 变化 → 命中率读数在切换轮失真（诊断风险,非真实多耗）**：
  `measureTranscript`（`context/ledger.ts:115`）只在遇到 compaction summary 时
  才判定旧 anchor 失效（`ledger.ts:11-15`）;charter 增删同样改写了真实 prompt
  前缀,但 ledger 不知情,仍沿用切换前那一步的 `inputTokens` 作 anchor。后果:
  charter 边界那一轮的真实 input（含全量重算）与账本估算对不上,`snapshot.hitRatio`
  （`context/index.ts:92`）会在该轮突然跳水,让人误以为"缓存经常出问题"。真实缓存
  只在状态翻转那一轮失效一次,读数的失真幅度大于实际损失。若要修读数(可选,不改
  真实缓存)：在 charter 出现/消失的轮次把它当作一次前缀失效事件(类比 summary),
  令 anchor 失效。列为可选,因它只修诊断口径、不修 §8 第二条的真实失效。
