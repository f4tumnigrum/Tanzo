...
```
src/shared/
  message-steps.ts     # 新：per-step 拆行/分组（splitAssistantSteps / groupAssistantSteps）
src/main/agent/context/
  ...
  compact/
    ...
    cut.ts             # 新：findCut/splitForCompaction（UI 域整行）+ splitModelTranscript（模型域）
...
src/main/agent/runtime/
  ...
  stream-runner.ts           # prepareStep 内联压缩；持久化边界 splitStepMessages
  run-persistence-registry.ts# steering 按 stepNumber 精确插位（D6-2 彻底修复）
src/main/database/
  per-step-migration.ts      # 迁移 22：存量多步 assistant 行拆分 + overlay 覆盖重映射
```
...
- **锚点即消息**：per-step 行（4.5）每行恰好携带一步 `metadata.steps[0].usage.inputTokens`，
  即 provider 报告的该步精确 prompt 体积；锚点自身增量 = 该步 outputTokens。
...
```ts
// UI 域（持久化 transcript，per-step 行 → 永远整行切）
function findCut(messages: TanzoUIMessage[], retainBudgetTokens: number): number | null
...
`retainBudget`，降级到 **step 组边界**（UI 域即 per-step 行之间，见 4.5）；扫描遇上一条
...
- UI 域永远整行切：`archivedIds` 永远是整行 id，无 mid-message 分裂（见 4.5）。
...
### 4.5 持久层：per-step 行（已实现）

现状的根源问题是"一条 UI 消息 ≠ 一个可切割单元"。实现落点：

- **拆行器**：`@shared/message-steps`（`splitAssistantSteps`）。首片段保留原 id，后续
  片段 id 为确定性的 `{baseId}::step-k`（同一输入永远产生同一组 id，id-based merge
  才能幂等）；每片段只携带自己那一步的 `metadata.steps[0]`（账本锚点形状），聚合
  `metadata.usage` 只在末片段。
- **写入边界**：`stream-runner.ts` 的 `onStepEnd`/`onEnd` 持久化回调处套
  `splitStepMessages`。**发往渲染器的 chunk 流不变**（SDK 单消息重建不受影响），
  拆分只发生在写库表示上。
- **渲染还原**：`groupAssistantSteps`（`message-list.tsx`）把连续 step 片段合并回一个
  视觉块；合并块取末片段 id（fork 按 id 切片时包含整条回复）。
- **收益**：`archivedIds` 永远是整行；overlay `covers_from_seq/covers_to_seq` 语义精确；
  `partitionAtCut` 的 mid-message 分裂、`randomUUID` 尾片段、`normalizeTailParts` 全部
  删除；**steering 精确插位**：`recordConsumedSteering` 携带 prepareStep 的
  `stepNumber`，merge 时插在首个 `steps[0].stepNumber > n` 的片段之前（D6-2 彻底闭环）。
- **迁移**：DB migration 22（`database/per-step-migration.ts`）：按最新 revision 的内容
  拆分存量多步 assistant 行、每会话密集重排 seq、overlay covers 范围重映射、丢弃失效的
  聚合 revision。
- `load / loadDisplay / loadArchived` 逻辑形状不变（overlay 之后取 tail 行）；
  `message-repo.ts` 本身零改动（行模型本就是一行一消息）。
...
| `withConsumedSteering` 重排 | run-persistence-registry.ts | 消费点 append |
| `partitionAtCut` mid-message 分裂族 | segments.ts / message-repo | per-step 行 |
...
| 2 | 持久层 per-step 行 + 注入持久化 + 迁移脚本 | message-repo 单测（load/loadDisplay/overlay）；迁移脚本对拷贝库跑幂等校验 |
...
- **per-step 行迁移**是最大的一刀（动持久层 + 渲染分组）。若需先止血，可先做阶段 1/3/4/5，
  阶段 2 暂以"模型域切点 + overlay 部分覆盖"过渡——但会保留 `partitionAtCut` 特殊情形族，
  不建议长期保留。
