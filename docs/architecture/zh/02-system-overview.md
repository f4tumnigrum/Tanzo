...
> 适用范围：高层组件与端到端数据流。最后核对：对照 `src/`（v0.2.4）当前实现。
...
│                                                                            │
│  App Shell: QueryClient · ThemeProvider · i18n · HashRouter · 可缩放 UI     │
...
│  platform/electron/*  唯一接触 window.electron 的层                         │
│                              │                                             │
└──────────────────────────────┼────────────────────────────────────────────┘
                               │  contextBridge: window.electron
┌──────────────────────────────┼────────────────────── PRELOAD ────────────┐
│  index.ts 组装 tanzoApi = { ...system, preferences, mcp, provider, chat,   │
│    policy, hooks, goal, git, changeSet, activity, skills, plugins,        │
│    slashCommand, fileMention, pet, browser }                             │
│                       —— 全部由 invoke() / subscribe() 两个原语构成         │
└──────────────────────────────┼────────────────────────────────────────────┘
                               │  ipcMain.handle / webContents.send
┌──────────────────────────────┼─────────────────────── MAIN (Node) ────────┐
│  index.ts 引导: 单实例锁 → 协议注册 → 预留 loopback 端口 → whenReady →       │
│                 模块工厂链                                                  │
│                                                                            │
│  agent/      createAgentModule  ← 系统核心                                 │
│    service(邮箱/入口) · runtime/(RunEngine, TurnLoop, streamText, 流批处理)│
│    context/(Section × Provider, 压缩) · tools/(内置+MCP+provider 合并) ·    │
│    policy + hooks(工具前置门) · subagent/ · skills/ · plugins/ · goal/ ·    │
│    presence/ · telemetry/ · repositories/ · ipc/                          │
│  provider/   createProviderModule  ProviderRuntime + 5 适配器 + 密钥安全   │
│  mcp/        createMcpModule        MCP 客户端生命周期 + Elicitation        │
│  database/   createDatabaseModule   better-sqlite3(WAL) + 迁移框架          │
│  preferences · system · wallpaper · file-mention · slash-command · pet     │
│                                                                            │
│  SQLite (唯一真源): workspaces / conversations /                          │
│    messages(追加日志) + message_revisions + compaction_overlays /          │
│    runs / run_steps / provider_* / mcp_servers / policy_* / skill_states / │
│    app_settings(含 hooks 状态) / plugin_* / ...                            │
└────────────────────────────────────────────────────────────────────────────┘
```
...
- `registerIpc` 是**幂等**的：内部先 `unregister?.()` 再注册；路由层的 `registerIpcHandlers`（`src/main/ipc/router.ts:33`）也在注册前先移除每个通道，重复调用安全。
- 依赖通过 `deps` 显式注入：`createAgentModule` 消费 `providerModule.service`、`mcpModule.service` 与 `databaseModule.db`，体现明确的依赖方向（`src/main/index.ts:235`）。
- 关闭顺序在 `before-quit` 里**刻意编排**：pet 窗口 → 轻量模块 → agent/mcp（await）→ provider → database 最后关（`src/main/index.ts:318`）。详见 [03 进程模型](./03-process-model.md)。
...
[main] ipc/chat.ts 校验 → service.submitMessage
   ...
   → 子代理对话? 后台驱动 : service.run(chatId, messages)
   → TurnLoop.run (≤10 次): 解析 def → 保存消息
        ...
        → RunEngine.run / startChatRun:
             ...
             → onStepEnd / onEnd → 持久化消息
             → for await chunk: deps.send(chatId, chunk, { runId })
─────────────────────────────────────────────────────────── IPC ──────────
[main→renderer] run-session-registry 批量合并 delta chunk(24ms 窗口)
...
| 关注点 | 选型 | 位置 |
|---|---|---|
| 桌面壳 | Electron 41 + electron-vite 6 | `electron.vite.config.ts` |
...
| 设置内嵌页 | Skills / Plugins / Providers / MCP / Usage 作为 Settings sections 嵌入 | `features/settings/model/sections.tsx` |
