...
- **I1 —— append-only 前缀**：两次压缩事件之间，每一步的 prompt 都是上一步 prompt 的严格前缀
  ...
  OpenAI/DeepSeek 前缀哈希、Gemini 隐式）生效的根基。
...
- **DeepSeek / Google** —— 自动前缀缓存，无显式标记。v1 的前缀冻结机制已删除——I1 使其
  不再必要。

工具序列化顺序经 `build-agent.ts` 的 `toolOrder`（排序）钉死，因为 Anthropic 缓存前缀包含
tools 块。

## 7. Per-step 持久化（设计 §4.5）

### 7.1 写入路径

AI SDK 把整个模型 pass 聚合为一条 assistant UIMessage（parts 以 `step-start` 分隔，usage 累积
在 `metadata.steps[]`）。`@shared/message-steps`（`splitAssistantSteps`）在持久化边界
（`stream-runner.ts` 的 `onStepEnd`/`onEnd`）把它拆为**每 step 组一行**：首片段保留原 id，
后续片段获得确定性 id（`{baseId}::step-k`），每片段恰好携带自己那一步的
`metadata.steps[0]`（账本锚点形状）。聚合 `metadata.usage` 只在末片段。发往渲染器的 chunk 流
不变——拆分只发生在写库表示上。

推论：

- 压缩切割永远覆盖整行；overlay `covers_from_seq/covers_to_seq` 语义精确；
- run 中 steering 持久化在模型实际看到的确切步位：`recordConsumedSteering` 携带 prepareStep
  的 `stepNumber`，merge 时插在首个 `steps[0].stepNumber` 超过它的片段之前（D6-2 彻底闭环）；
- 账本锚点即最新 per-step 行的上报 `inputTokens` 加其自身输出。

### 7.2 展示路径

渲染器重组连续 step 片段（`groupAssistantSteps`，应用于 `message-list.tsx`），多步回复渲染为
一个块，与直播流视图一致。合并块取**末**片段 id，fork（按目标 id 切片）因此包含整条回复。

### 7.3 迁移

DB migration 22（`database/per-step-migration.ts`）用同一个 `splitAssistantSteps` 拆分历史
多步 assistant 行、每会话密集重排 seq、把压缩 overlay 覆盖范围重映射到新 seq、丢弃失效的聚合
revision（日志投影会以 revision COALESCE 覆盖基础行）。
