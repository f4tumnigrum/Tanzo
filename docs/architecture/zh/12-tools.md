...
4. 编排工具，按条件加入：`skill`、`todo` 恒有；`askQuestion` 仅主 agent；`shellBackgroundTools`；`subagentTools` 仅当 `canDelegate`；子代理 `note` 工具仅当自身是子代理；`goalTools`（`updateGoal`）仅主 agent；`exitPlanMode` 仅计划模式或此前有 `exitPlanMode` 审批时。
...
编排工具同规则：`skill`、`askQuestion`、`await`、`tasks`、`note`、`shellPoll`、`shellList` 为 `read`（可自动放行）；`shellStart` / `shellWrite` / `shellStop`、`spawn` / `steer` / `cancel`、`todo`、`updateGoal` 为 `exec`（需审批）。`exitPlanMode` 无 `kind`——恒需审批（见 [13 策略与审批](./13-policy-and-approval.md)）。
...
- `await` 以 `settle: 'all' | 'first'` 与可选 `timeoutMs` 阻塞在 `deps.awaitTask`（超时后任务继续跑）。它是**事件驱动而非轮询**：等待过程在「任务结算」「note 唤醒」「可选超时」之间竞速。当被等待的任务发出 note（`waitForNoteTask`）时，`await` 提前返回，无需父代理主动轮询。`notedTasks` 精确列出**本次调用期间哪些被等待任务产生了新 note**——通过等待开始时对每个任务记录 note 计数基线、返回前比对得出，因此在多任务场景下精确，且能承受「一个任务结算、另一个同帧发 note」的竞态（单个布尔无法表达）。note 唤醒或超时时，`pending` 为每个未完成任务的结构化快照——`{ task, status, phase, latestNote, updatedAt }`——其中 `phase` 由子代理的工具活动自动派生（非自报）。未知 id 在 `unknown` 字段显式返回而非静默丢弃；结果携带 `failureKind`（`app-restart` | `logic-error` | `await-cancelled`）与累积的 `notes`。每次 `await` 将其 settle/note 等待者绑定到一个本地 `AbortController`，使被放弃的等待者及时脱钩而非跨多次 await 累积。`kind: 'read'`。
...
子代理就是一个标准 agent loop：它的**交付物就是其运行自然收敛时的最后一条 assistant 消息**，由 `completeTask` 经 `lastAssistantText` 原样取用——没有单独的「提交」步骤，也没有 `explicit`/`inferred` 置信度分层。若该最后文本为空，任务**失败**（`failureKind: 'logic-error'`）而非以空 summary 完成。子代理唯一的回报是 **note**，经 `note` 工具（`tools/subagent-control.ts`，`note → addTaskNote`；仅为子代理加入）：一个中途信号——意外、阻塞、方向分叉、部分结果——随 `pending.latestNote` 与 `result.notes` 回流，**并唤醒任何正在等待该任务的父代理**（`wake-note` 效果只唤醒 note 等待者而不结算）。note 不是交付物、也不结束运行。实时进度无需自报：`runStreamPass` 将该运行的 `onTrace` 接到 `setPhaseByChat`，子代理每调用一个工具即成为其当前 `phase`。进度经 `chat:task-event` 到 UI（见 [04 跨进程契约](./04-ipc-and-contracts.md)）。
...
**计划模式限制**：`plan` 模式下只提供只读子代理。子代理"安全只读"当且仅当其 `allowedTools` 全在 `READ_ONLY_SUBAGENT_TOOLS`（`fileRead`、`glob`、`grep`、`skill`、`await`、`tasks`、`note`、`shellPoll`、`shellList`、`web_search`）。不可用类型带原因「plan mode allows read-only sub-agents only」。
