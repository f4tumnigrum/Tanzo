...
只有一个模块 `tanzoMigrations`（moduleName `'tanzo'`，`database/schema.ts:418`），在 `src/main/index.ts:217`（`migrations: [tanzoMigrations]`）接线——唯一注册的模块。其文件为 v1 `initial_schema`、v19 `plugin_states`、v20 `plugin_marketplaces`（v2–v18 缺口反映扁平化历史：初始 schema 已含至 v18 的一切）。
